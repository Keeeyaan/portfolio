  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="mt-6 flex flex-col md:flex-row md:justify-between items-center text-center"
      >
          ...
          <ul className="text-lightsecondary dark:text-secondary ">
            {socmeds.map((socmed) => (
              <li key={socmed.name} className="mb-2 cursor-pointer">
                <p onClick={() => window.open(socmed.links, "_blank")}>
                  {socmed.name}
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
      ...
      <motion.div
        variants={fadeIn("", "", 0.1, 1)}
        className=" justify-center md:justify-end mt-4 flex gap-4 items-center "
      >
        {socmeds.map((socmed) => (
          <a key={socmed.name} className="cursor-pointer">
            <img
              src={socmed.icon}
              onClick={() => window.open(socmed.links, "_blank")}
